...
M6Apeak.MultiRep.step1 <- function(Counts, sf, bins,
  ...
  for (j in 1:length(blocks)) {
    ...
    #lfc = log((dat[, 2]/sf[2] + c0[j])/(dat[, 1]/sf[1] + c0[j]))
    ...
    tmp = findBumps(chr = bins$chr, pos = bins$start, strand = bins$strand,
                    ...
                    count = dat)
    Bumps[[j]] = tmp[tmp$counts > lowcount, ] ### remove low count
  }
  ...
  if(nrow(thispeak) >= 1){
    ...
    if(nrow(thiscount) >= 2){
      ...
      idx = which(rowSums(thiscount[, seq(2, ncol(thiscount), 2)])  > 50)
      tmp = Peaks[idx, ]
...
M6Apeak.MultiRep.step2 <- function(Candidates, sf, mu.cutoff = 0.5, addDEseq2 = FALSE){
  ...
  if(!addDEseq2){
    ...
    ## deseq2
    thisstat = m6Apeak.RNAseq(counts = thiscount, sf = sf)$stat
    tmp = 1 - pnorm(thisstat)
    thispeak$DESeq_pval = tmp
    thispeak$DESeq_score = p.adjust(tmp, method = "fdr")
    thispeak$DESeq_stat = thisstat

    ## proposed
    res = M6Apeak(mat = as.matrix(thiscount), sf = sf, cutoff = mu.cutoff)
    thispeak$NB_score = p.adjust(res$pvals, method = "fdr")
    thispeak$NB_stat = res$stats
    thispeak$NB_pval = res$pval
    thispeak$NB_mu = res$mu
