...
M6Apeak.MultiRep.step2 <- function(Candidates, sf, mu.cutoff,
  ...
  if(WhichThreshold == "pval"){
    flag <- thispeak$pvals < pval.cutoff
  }else if(WhichThreshold == "fdr"){
    flag <- thispeak$p.adj < fdr.cutoff
  }else if(WhichThreshold == "lfc"){
    flag <- thispeak$lg.fc >= lfc.cutoff
  } else if(WhichThreshold == "pval_lfc"){
    flag <- (thispeak$pvals < pval.cutoff & thispeak$lg.fc >= lfc.cutoff)
  }else if(WhichThreshold == "fdr_lfc"){
    flag <- (thispeak$p.adj < fdr.cutoff & thispeak$lg.fc >= lfc.cutoff)
  }
  peaks = thispeak[flag, ]
  ####
